...
window.app = {
  createConversation: function (roomid) {
  ...
  joinConversation: function (roomid) {
   // var convId = $('#convId').val();
    nextRTC.join(roomid);
  },
...
var nextRTC = new NextRTC({
  wsURL: 'wss://' + window.location.hostname+':8443' + '/signaling',
	//wsURL: 'wss://' + 'localhost:8443' + '/signaling',
  mediaConfig: {
    video: true,
    audio: false,
  },
...
//  var dest = $("#template").clone().prop({id: 'local'});
 ...
 // dest[0].muted = true;
	...
	videoSource = window.URL.createObjectURL(stream.stream)
...
nextRTC.on('remoteStream', function (stream) {
  ...
  //dest[0].srcObject = stream.stream;
	...
	video.autoplay = true;
  	...
  	video.setAttribute("class", "remotestream");
	// point the "src" attr. of the <video> element to the generated stream URL, to relay the data from the webcam and microphone back to the user 
	video.src = videoSource;
	// add the <video> element to the end of the current page
	$("#container").append(video);
